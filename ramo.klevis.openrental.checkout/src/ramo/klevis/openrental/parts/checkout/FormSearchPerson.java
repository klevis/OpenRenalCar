...
public class FormSearchPerson extends Shell {
...
//	}
	...
	public FormSearchPerson(Shell shell, final FormCheckOut formCheckOut) {
		// TODO Auto-generated constructor stub
		super(shell, SWT.CLOSE | SWT.APPLICATION_MODAL);
		setText("Zgjidh personin");
		setSize(734, 300);
		...
		btnZgjidhKlientin.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selection.isEmpty() == false) {
					...
					FormCustomer formCustomer = formCheckOut.getFormCustomer();
					formCustomer.dispose();
					formCheckOut.getFormAdditionalDirver().dispose();
					formCheckOut.getFormMainDriver().dispose();
					FormSearchPerson.this.close();
					FormSearchPerson.this.dispose();
					setSelected(true);
				} else {
					setSelected(false);
	...
	protected DataBindingContext initDataBindings() {
		...
		IObservableMap[] observeMaps = PojoObservables.observeMaps(listContentProvider.getKnownElements(), Customer.class, new String[]{"pafirstname", "palastname", "nrPikeve", "draddress", "drhphone", "drpasportid", "drcountry", "drcity"});
		tableViewer.setLabelProvider(new ObservableMapLabelProvider(observeMaps));
		tableViewer.setContentProvider(listContentProvider);
		//
		WritableList writableList = new WritableList(listCustomer, Customer.class);
		tableViewer.setInput(writableList);
		//
		IObservableValue textObserveTextObserveWidget = SWTObservables.observeText(text, SWT.Modify);
		IObservableValue personSearchNameObserveValue = PojoObservables.observeValue(getPersonSearch(), "name");
		bindingContext.bindValue(textObserveTextObserveWidget, personSearchNameObserveValue, null, null);
		//
		IObservableValue text_1ObserveTextObserveWidget = SWTObservables.observeText(text_1, SWT.Modify);
		IObservableValue personSearchSurnameObserveValue = PojoObservables.observeValue(getPersonSearch(), "surname");
