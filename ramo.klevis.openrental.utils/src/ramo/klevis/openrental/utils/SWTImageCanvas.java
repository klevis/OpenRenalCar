...
public class SWTImageCanvas extends Canvas {
	 ...
	 */
	public void dispose() {
		if (getSourceImage() != null && !getSourceImage().isDisposed()) {
			getSourceImage().dispose();
		}
		if (getScreenImage() != null && !getScreenImage().isDisposed()) {
	...
	private void paint(GC gc) {
		...
		if (getSourceImage() != null) {
			...
			Rectangle imageBound = getSourceImage().getBounds();
			imageRect = imageRect.intersection(imageBound);
			Rectangle destRect = SWT2Dutil.transformRect(transform, imageRect);

			if (getScreenImage() != null)
				getScreenImage().dispose();
			setScreenImage(new Image(getDisplay(), clientRect.width,
					clientRect.height));
			GC newGC = new GC(getScreenImage());
			newGC.setClipping(clientRect);
			newGC.drawImage(getSourceImage(), imageRect.x, imageRect.y,
					imageRect.width, imageRect.height, destRect.x, destRect.y,
					destRect.width, destRect.height);
			newGC.dispose();

			gc.drawImage(getScreenImage(), 0, 0);
		} else {
	...
	public Image loadImage(String filename) {
		...
		setSourceImage(new Image(getDisplay(), filename));
		showOriginal();
	...
	public ImageData getImageData() {
		return getSourceImage().getImageData();
	}
	...
	public void setImageData(ImageData data) {
		if (getSourceImage() != null)
			getSourceImage().dispose();
		if (data != null)
			setSourceImage(new Image(getDisplay(), data));
		if (data == null) {
			setSourceImage(null);
